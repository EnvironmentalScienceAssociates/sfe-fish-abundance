function(input, output, session) {
  ...
  output$map = renderLeaflet({
    leaflet(options = leafletOptions(attributionControl = FALSE)) |>
      ...
      addPolygons(data = boundary, weight = 3, fillOpacity = 0)
  })
    ...
    if (nrow(sourcePoints()) > 0) {
      proxy |>
        ...
        addLegend(
          "bottomright",
          pal = pal,
          values = unique(samplesSubSource()$Source),
          title = "Data Source",
          opacity = 1
        )
    }
  ...
  output$messageButton <- renderUI({
    if (is.null(rv$shape)) {
      helpText(
        "Use map drawing tools to select samples to include in abundance tally."
      )
    } else {
  ...
  output$taxaFilters <- renderUI({
    ...
    tagList(
      checkboxGroupInput(
        "taxa_filters",
        "Filter Taxa List",
        choices = taxa_filters,
        selected = taxa_filters
      ),
