...
dt7 <-read.csv(infile7,header=F 
               ...
               , col.names=c(
                 ...
                 "PeriodOfRecordTo",     
                 "MonitoringType",     
                 "MethodCode"    ), check.names=TRUE)

unlink(infile7)

# Fix any interval or ratio columns mistakenly read in as nominal and nominal columns read as numeric or dates read as strings

if (class(dt7$StationCode)!="factor") dt7$StationCode<- as.factor(dt7$StationCode)
if (class(dt7$StationName)!="factor") dt7$StationName<- as.factor(dt7$StationName)
if (class(dt7$StationNumber)!="factor") dt7$StationNumber<- as.factor(dt7$StationNumber)
...
if (class(dt7$PeriodOfRecordFrom)!="factor") dt7$PeriodOfRecordFrom<- as.factor(dt7$PeriodOfRecordFrom)
if (class(dt7$PeriodOfRecordTo)!="factor") dt7$PeriodOfRecordTo<- as.factor(dt7$PeriodOfRecordTo)
if (class(dt7$MonitoringType)!="factor") dt7$MonitoringType<- as.factor(dt7$MonitoringType)
if (class(dt7$MethodCode)!="factor") dt7$MethodCode<- as.factor(dt7$MethodCode)
...
if (class(dt8$OrganismCode)!="factor") dt8$OrganismCode<- as.factor(dt8$OrganismCode)
if (class(dt8$IEPFishCode)!="factor") dt8$IEPFishCode<- as.factor(dt8$IEPFishCode)
if (class(dt8$CommonName)!="factor") dt8$CommonName<- as.factor(dt8$CommonName)
if (class(dt8$Native)!="factor") dt8$Native<- as.factor(dt8$Native)
if (class(dt8$Phylum)!="factor") dt8$Phylum<- as.factor(dt8$Phylum)
if (class(dt8$Class)!="factor") dt8$Class<- as.factor(dt8$Class)
if (class(dt8$Order)!="factor") dt8$Order<- as.factor(dt8$Order)
...
dt8$Native <- as.factor(ifelse((trimws(as.character(dt8$Native))==trimws("NA")),NA,as.character(dt8$Native)))
dt8$Phylum <- as.factor(ifelse((trimws(as.character(dt8$Phylum))==trimws("NA")),NA,as.character(dt8$Phylum)))
dt8$Class <- as.factor(ifelse((trimws(as.character(dt8$Class))==trimws("NA")),NA,as.character(dt8$Class)))
dt8$Order <- as.factor(ifelse((trimws(as.character(dt8$Order))==trimws("NA")),NA,as.character(dt8$Order)))
dt8$Family <- as.factor(ifelse((trimws(as.character(dt8$Family))==trimws("NA")),NA,as.character(dt8$Family)))
dt8$Genus <- as.factor(ifelse((trimws(as.character(dt8$Genus))==trimws("NA")),NA,as.character(dt8$Genus)))
...
if (class(dt9$SampleID)!="factor") dt9$SampleID<- as.factor(dt9$SampleID)
if (class(dt9$EventID)!="factor") dt9$EventID<- as.factor(dt9$EventID)
if (class(dt9$OrganismCode)!="factor") dt9$OrganismCode<- as.factor(dt9$OrganismCode)
if (class(dt9$Count)=="factor") dt9$Count <-as.numeric(levels(dt9$Count))[as.integer(dt9$Count) ]               
if (class(dt9$Count)=="character") dt9$Count <-as.numeric(dt9$Count)
...
dt2 |> 
  ...
  dplyr::left_join(dplyr::select(dt7, Station = StationCode, Latitude, Longitude)) |>
